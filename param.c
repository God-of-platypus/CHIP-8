    ...
    for (int32_t i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "--machine") == 0)
        {
            ...
            if (strcmp(argv[i + 1], "COSMAC") && strcmp(argv[i + 1], "CHIP-48"))
            {
                fprintf(
                    stderr,
                    "The machine %s is unknown: please use COSMAC or CHIP-48\n",
                    argv[i + 1]);
                return 1;
            }
            param.machine = true;
        ...
        {
            fprintf(stderr,
                    "The extension of %s is wrong. Please use a file with the "
                    "following extension: .ch8\n",
                    argv[i]);
            return 1;
        }
        else
...
{
    fprintf(stdout,
            "Usage: ./chip-8 [--log] [--help] [--log_file file_to_log_to] "
            "[--machine machine_name] rom\n");
    fprintf(
        stdout,
        "This program only uses files with the following extension: .ch8\n");
    fprintf(stdout,
            "This program has two option for which machine to use: COSMAC and "
            "CHIP-48\n");
